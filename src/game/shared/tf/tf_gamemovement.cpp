...
bool CTFGameMovement::StunMove()
{
	...
	if ( m_pTFPlayer->IsViewingCYOAPDA() || ( TFGameRules() && TFGameRules()->InMatchStartFreeze() ) )
	{
		mv->m_flForwardMove = 0.f;
		mv->m_flSideMove = 0.f;
		mv->m_flUpMove = 0.f;
...
void CTFGameMovement::AirDash( void )
{
	...
	if ( pWpn && ( !TFGameRules()->IsBetaActive() || GAMEMOVEMENT_NONSUB_CURTIME >= pWpn->GetLastReadyTime() ) )
	{
...
void CTFGameMovement::WalkMove( void )
{
	...
	if ( trace.fraction == 1.0f )
	{
		...
		if ( tf_showspeed.GetInt() == 2 && baseVelocity.IsZero() && ( flTestSpeed > ( flAdjustedMaxSpeed + 1.0f ) ) )
		{
	...
	vecTestVelocity.z = 0.0f;
	float flTestSpeed = VectorLength( vecTestVelocity );
	if ( tf_showspeed.GetInt() == 1 && baseVelocity.IsZero() && ( flTestSpeed > ( flAdjustedMaxSpeed + 1.0f ) ) )
	{
		Msg( "Step Max Speed < %f\n", flTestSpeed );
	}
...
*/
	if ( m_pTFPlayer->m_Shared.InCond( TF_COND_PARACHUTE_ACTIVE ) )
	{
		...
		if ( flSpeedXY < mv->m_flMaxSpeed && ( flDot >= 0.0f || flSpeedXY < 5.0f ) )
		{
			...
			flAccel = Min( flAccel, flDiff );
			mv->m_vecVelocity.x += wishdir.x * flAccel;
			mv->m_vecVelocity.y += wishdir.y * flAccel;
		}
	}
	else
	{
